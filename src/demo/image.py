...
import conf
from datasets import TobaccoImgDataset, RvlImgDataset
...
_, c = np.unique(np.array(d_train.targets), return_counts=True)
...
plt.yticks(range(len(cm)), conf.tobacco_labels, rotation=70)
plt.xlabel('Predicted Category')
plt.xticks(range(len(cm)), conf.tobacco_labels, rotation=70)
